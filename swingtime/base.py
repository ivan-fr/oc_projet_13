...
from swingtime import settings
...
def serialize(rule_or_recurrence):
    ...
    def serialize_rule(rule):
        values = [(u'FREQ', [Rule.frequencies[rule.freq]])]

        if rule.interval != 1:
            values.append((u'INTERVAL', [str(int(rule.interval))]))
        if rule.wkst:
            values.append((u'WKST', [
                Rule.weekdays[getattr(rule.wkst, 'number', rule.wkst)]]))
...
def rule_to_text(rule, short=False):
    ...
    def get_positional_weekdays(_rule):
        _items = []
        if _rule.bysetpos and _rule.byday:
            for setpos in _rule.bysetpos:
                for byday in _rule.byday:
...
MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY = (
    MO, TU, WE, TH, FR, SA, SU) = WEEKDAYS = list(
    map(lambda n: Weekday(n), range(7)))
...
def deserialize(text, include_dtstart=True):
    ...
    return Recurrence(dtstart, dtend,
                      rrules,
                      exrules, rdates,
                      exdates,
                      include_dtstart)
