    ...
    copy: function () {
        return new recurrence.Recurrence({
            'rrules': recurrence.array.foreach(
                this.rrules, function (item) {
                    return item.copy();
                }),
            'exrules': recurrence.array.foreach(
                this.exrules, function (item) {
                    return item.copy();
                }),
            'rdates': recurrence.array.foreach(
                this.rdates, function (item) {
                    return item.copy();
                }),
            'exdates': recurrence.array.foreach(
                this.exdates, function (item) {
                    return item.copy();
                })
        });
    },
...
recurrence.serialize = function (rule_or_recurrence) {
    ...
    var serialize_rule = function (rule) {
        ...
        if (rule.hours != null)
            values.push(['BYHOUR', [String(rule.hours)]]);
        if (rule.minutes != null)
            values.push(['BYMINUTE', [String(rule.minutes)]]);
        if (rule.wkst)
                ...
                recurrence.array.foreach(
                    params, function (item) {
                        ...
                        if (key == 'FREQ') {
                            ...
                            options['hours'] = parseInt(value[0], 10);
                        } else if (key == 'BYMINUTE') {
                            options['minutes'] = parseInt(value[0], 10);
                        } else if (key == 'COUNT') {
                            options[param] = parseInt(value[0], 10);
                        } else if (key == 'UNTIL') {
