...
import swingtime
from swingtime import forms
from swingtime.compat import Creator
...
class RecurrenceField(fields.Field):
    ...
    def to_python(self, value):
        if value is None or isinstance(value, swingtime.Recurrence):
            return value
        value = super(RecurrenceField, self).to_python(value) or u''
        return swingtime.deserialize(value, self.include_dtstart)
    ...
    def get_prep_value(self, value):
        if not isinstance(value, string_types):
            value = swingtime.serialize(value)
        return value
